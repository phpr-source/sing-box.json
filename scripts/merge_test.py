...
def calculate_js_divergence(p: Dict[str, float], q: Dict[str, float]) -> float:
    ...
    kl_pm = sum(p[k] * math.log2(p[k] / m[k]) for k in p if p[k] > 0 and m[k] > 0)
    kl_qm = sum(q[k] * math.log2(q[k] / m[k]) for k in q if q[k] > 0 and m[k] > 0)
        ...
        if not self.script_type:
            ...
            for char in self.normalized:
                try:
                    ...
                    if script in ('LATIN', 'CYRILLIC', 'GREEK', 'ARABIC', 'HEBREW', 'CJK', 'HANGUL'):
                        scripts.add(script)
                except:
                    pass
            if scripts:
...
class ResourceMonitor:
    __slots__ = ('_threshold', '_enabled', '_start_time', '_max_duration', '_check_count')
...
class VersionedBDDNode:
    __slots__ = ('var', 'low', 'high', 'hash_val', '_ref_count', '_generation', '_migrated_to')
...
class TransactionalBDDEngine:
    __slots__ = ('var_map', 'var_counter', 'true_node', 'false_node', '_lock', '_op_cache', '_node_count', 
                 ...
                 '_rebuild_lock', '_reordering_active', '_generation', '_transaction_active')
    ...
    def neg(self, node: VersionedBDDNode) -> VersionedBDDNode:
        ...
        with self._op_cache_lock:
            ...
            if result is not None:
                return result
            result = VersionedBDDNode(node.var, self.neg(node.low), self.neg(node.high), self._generation)
        ...
        with self._op_cache_lock:
            ...
            if f.var == g.var:
                result = VersionedBDDNode(f.var, 
                              self._apply_op(f.low, g.low, op),
                              self._apply_op(f.high, g.high, op), self._generation)
            elif f.var < g.var:
                result = VersionedBDDNode(f.var,
                              self._apply_op(f.low, g, op),
                              self._apply_op(f.high, g, op), self._generation)
            else:
                result = VersionedBDDNode(g.var,
                              self._apply_op(f, g.low, op),
                              self._apply_op(f, g.high, op), self._generation)
    ...
    def sat_count(self, node: VersionedBDDNode, n_vars: int) -> int:
        ...
        if node is self.true_node:
            return 1 << n_vars
        count_low = self.sat_count(node.low, n_vars - 1)
        count_high = self.sat_count(node.high, n_vars - 1)
    ...
    def sifting_reorder(self):
        with self._rebuild_lock:
            if self._reordering_active or self._transaction_active:
                return
            self._reordering_active = True
        
        try:
            with self._var_order_lock:
                if len(self._var_order) < 2:
                    return
                
                current_size = self._count_nodes()
                if current_size < self._gc_threshold // 2:
                    return
                
                self.begin_transaction()
                
                best_order = list(self._var_order)
                best_size = current_size
                
                for i in range(len(self._var_order)):
                    var = self._var_order[i]
                    best_pos = i
                    min_size = self._estimate_size()
                    
                    for direction in [-1, 1]:
                        current_order = list(best_order)
                        current_pos = i
                        
                        while 0 <= current_pos + direction < len(current_order):
                            swap_pos = current_pos + direction
                            current_order[current_pos], current_order[swap_pos] = current_order[swap_pos], current_order[current_pos]
                            
                            new_size = self._estimate_size_for_order(current_order)
                ...
                final_size = self._count_nodes()
                if final_size > best_size * 1.1:
                    pass
        finally:
            self._reordering_active = False
    ...
    def _estimate_size(self) -> int:
        return len(self._var_order) * 10
    
    def _estimate_size_for_order(self, order: List[str]) -> int:
        return len(order) * 10
...
class BDDRuleVerifier:
    ...
    def encode_domain_rule(self, rule: DomainRule) -> VersionedBDDNode:
        with self._lock:
            var_name = f"dom_{rule.normalized}_{rule.match_type.value}"
            ...
            result = self._domain_vars[var_name]
...
class SMTVerifier:
    __slots__ = ('enabled', '_z3_available', '_solver_cache', '_config', '_timeout_stages', 
                 '_process_pool', '_pool_size', '_stop_event')
        ...
        try:
            ...
            for timeout_idx, timeout_ms in enumerate(self._timeout_stages):
                ...
                if self._pool_size == 0:
                    result = self._verify_worker(parent_strs, child_strs, timeout_ms)
                    is_subset, confidence, used_time, status = result
                    if status == "unknown":
                        confidence = 0.5
    ...
    def _verify_worker(self, parent_rules_str, child_rules_str, timeout_ms):
        if not HAS_Z3:
        ...
        try:
            ...
            def timeout_handler(signum, frame):
                raise TimeoutError("SMT solver timeout")
            ...
            if not parent_exprs or not child_exprs:
                signal.alarm(0)
                return True, 1.0, timeout_ms, "trivial"
            ...
            if result == z3.sat:
                return False, 1.0, timeout_ms, "sat"
            elif result == z3.unsat:
                return True, 1.0, timeout_ms, "unsat"
            else:
                return False, 0.5, timeout_ms, "unknown"
        except TimeoutError:
            return False, 0.0, timeout_ms, "timeout"
        except Exception as e:
            return False, 0.0, timeout_ms, f"error:{str(e)}"
...
class DomainTrie:
    ...
    class Node:
        __slots__ = ('children', 'types', 'terminal', 'wildcard', '_lock')
        def __init__(self):
            self.children = {}
            self.types = set()
            self.terminal = False
            ...
            for part in reversed(parts):
                with node._lock:
                    ...
                    if match_type == MatchType.WILDCARD:
                        node.wildcard = True
                        node.types.add(MatchType.WILDCARD)
                    elif match_type == MatchType.SUFFIX:
                        node.types.add(MatchType.SUFFIX)
            
            with node._lock:
                node.terminal = True
                node.types.add(match_type)
    ...
    def is_covered(self, domain: str, match_type: MatchType = MatchType.EXACT) -> Tuple[bool, MatchType, int]:
        cache_key = f"{domain}:{match_type.value}:{self._version}"
        with self._cache_lock:
...
class StrictPatriciaTrie:
    ...
    class Node:
        __slots__ = ('left', 'right', 'is_terminal', 'prefix_len')
        def __init__(self):
            self.left = None
            self.right = None
...
class IntervalTreeNode:
    ...
    def __init__(self, intervals: List[Tuple[int, int, Any]], depth: int = 0, max_depth: int = 64):
        self.intervals = []
        self.left = None
        self.right = None
        self.by_start = []
...
class SweepLineCIDRManager:
    __slots__ = ('_patricia', '_networks', '_lock', 'enable_adjacent_merge', '_interval_tree', '_version')
    ...
    def __init__(self, enable_adjacent_merge: bool = True, use_interval_tree: bool = True):
        ...
        self._interval_tree = IntervalTree(max_depth=48) if use_interval_tree else None
        self._version = 0
    ...
    @staticmethod
    def subtract(base_nets: List[Union[ipaddress.IPv4Network, ipaddress.IPv6Network]], 
                 ...
                 strict_zero_loss: bool = True) -> List[str]:
        ...
        try:
            v4_result = SweepLineCIDRManager._subtract_version(v4_base, v4_excl, 32, max_fragments // 2)
            remaining = max_fragments - len(v4_result)
            v6_result = SweepLineCIDRManager._subtract_version(v6_base, v6_excl, 128, remaining)
            total_result = v4_result + v6_result
            
            if enable_adjacent_merge:
                total_result = SweepLineCIDRManager._merge_adjacent_cidrs(total_result)
            
            if len(total_result) > max_fragments:
                if enable_approximation and len(total_result) > approximation_threshold:
                    total_result, loss_rate = SweepLineCIDRManager._hierarchical_supernet_with_loss(total_result, max_fragments)
            ...
            if enable_approximation and not strict_zero_loss:
                if e.loss_rate <= max_loss_rate:
                    merged, _ = SweepLineCIDRManager._hierarchical_supernet_with_loss(base_nets, max_fragments)
                    return [str(n) for n in merged]
            raise
    
    @staticmethod
    def _subtract_version(base: List, exclude: List, width: int, max_frag: int) -> List:
        if not base:
            return []
        if not exclude:
        ...
        for net in base:
            ...
            events.append((start, SweepLineCIDRManager.PRIORITY['BS'], 0))
            if end < max_addr:
                events.append((end + 1, SweepLineCIDRManager.PRIORITY['BE'], 0))
        
        for net in exclude:
            start = int(net.network_address)
            end = min(int(net.broadcast_address), max_addr)
            events.append((start, SweepLineCIDRManager.PRIORITY['ES'], 1))
            if end < max_addr:
                events.append((end + 1, SweepLineCIDRManager.PRIORITY['EE'], 1))
        ...
        for pos, prio, is_excl in events:
            ...
            if is_excl:
                if prio == SweepLineCIDRManager.PRIORITY['ES']:
                    exclude_depth += 1
                else:
                    exclude_depth -= 1
            else:
                if prio == SweepLineCIDRManager.PRIORITY['BS']:
                    base_depth += 1
                else:
                    base_depth -= 1
            prev_pos = pos
        ...
        for start, end in result_intervals:
            cidrs.extend(SweepLineCIDRManager._range_to_cidrs(start, end, width))
        
        return list(ipaddress.collapse_addresses(cidrs))
    
    @staticmethod
    def _hierarchical_supernet_with_loss(networks: List[Union[ipaddress.IPv4Network, ipaddress.IPv6Network]], 
                                         target_count: int) -> Tuple[List[Union[ipaddress.IPv4Network, ipaddress.IPv6Network]], float]:
        ...
        if len(merged) > target_count:
            ...
            for prefix_len in range(merged[0].prefixlen, min_prefix - 1, -1):
                ...
                merged = SweepLineCIDRManager._merge_adjacent_cidrs(aggregated)
                if len(merged) <= target_count:
                    break
    ...
    @staticmethod
    def _merge_adjacent_cidrs(networks: List[Union[ipaddress.IPv4Network, ipaddress.IPv6Network]]) -> List[Union[ipaddress.IPv4Network, ipaddress.IPv6Network]]:
        if not networks:
            return networks
        ...
        merged_v4 = SweepLineCIDRManager._merge_adjacent_version(v4_nets, 32)
        merged_v6 = SweepLineCIDRManager._merge_adjacent_version(v6_nets, 128)
        
        return merged_v4 + merged_v6
    
    @staticmethod
    def _merge_adjacent_version(nets: List, width: int) -> List:
        if len(nets) < 2:
            return nets
        ...
        if len(merged) < len(nets):
            return SweepLineCIDRManager._merge_adjacent_version(merged, width)
        return merged
    
    @staticmethod
    def _range_to_cidrs(start: int, end: int, width: int) -> List:
        cidrs = []
        current = start
        max_ip = (1 << width) - 1
        
        while current <= end and current <= max_ip:
            if current == 0:
...
class ProvenanceLogger:
    ...
    def _flush(self):
        batch = {f"prov_{i}": entry for i, entry in enumerate(self._activity_log)}
...
class MerkleClock:
    __slots__ = ('clock', '_merkle_tree', '_lock', '_hash_cache')
    ...
    def merge(self, other: 'MerkleClock'):
        with self._lock:
            ...
            for n, c in other.clock.items():
                if c > self.clock.get(n, 0):
                    self.clock[n] = c
                    self._merkle_tree.update_leaf(n, f"{n}:{c}")
                    changed = True
            if changed:
                self._hash_cache.clear()
            return self
...
class IncrementalMerkleTree:
    ...
    def _get_path_to_root(self, leaf_index: str) -> List[str]:
        ...
        current_level = 0
        current_pos = list(self.leaves.keys()).index(leaf_index) if leaf_index in self.leaves else 0
...
class IndexedSource:
    ...
    def from_parsed(cls, parsed: ParsedRuleSet, config: MergeConfig, bdd_engine: Optional[TransactionalBDDEngine] = None):
        ...
        ip_trie = StrictPatriciaTrie()
        interval_tree = IntervalTree(max_depth=48) if config.enable_interval_tree else None
        etld_set = set()
        
        bloom_size, bloom_k = optimal_bloom_size(max(len(parsed.domain_rules) + len(parsed.ip_rules), 100), config.bloom_target_fpp)
...
class StrictConflictResolver:
    __slots__ = ('config', '_entropy_cache', '_ngram_analyzer', '_policy_engine', '_tiered_strategy')
    ...
    def _temporal_compare(self, src_a: VerifiedSource, src_b: VerifiedSource) -> int:
        ...
        time_diff = 0
        if hasattr(src_a.indexed, 'last_modified') and hasattr(src_b.indexed, 'last_modified'):
    ...
    def resolve(self, sources: List[VerifiedSource], min_score: float) -> Dict[str, List[str]]:
        ...
        for src in sources:
            tier = self._tiered_strategy.select_tier(len(src.indexed.domain_rules) + len(src.indexed.ip_rules), 100.0)
            orig = src.lineage_info.originality if src.lineage_info else 1.0
            base_weight = src.weight * orig
            
            age_days = (current_time - 0) / 86400
        ...
        for weight, rtype, value, is_excl, src, rule_obj in all_rules:
            ...
            if is_excl:
                exclusions_map[key] = (weight, src)
            else:
                if key in accepted_domains or key in [(k[0], k[1]) for k in accepted_ips]:
                    conflict_map[key].append((src.url, weight, False))
                    continue
                
                if rtype == 'domain':
                    spec = rule_obj.specificity_score if isinstance(rule_obj, DomainRule) else 0
                    accepted_domains[value] = (weight, src, False, spec, rule_obj)
                    conflict_map[key].append((src.url, weight, False))
                elif rtype == 'ip_cidr':
                    try:
                        net = ipaddress.ip_network(value, strict=False)
                        accepted_ips.append((weight, net, False, src))
                        conflict_map[key].append((src.url, weight, False))
                    except ValueError:
                        continue
        ...
        if self._policy_engine:
            for key, sources_info in conflict_map.items():
                if len(sources_info) > 1:
                    ...
                    if not should_include and key in accepted_domains:
                        del accepted_domains[key]
        ...
        for value, (weight, src, is_excl, spec, rule_obj) in accepted_domains.items():
            if not is_excl:
        ...
        optimized = domain_trie.optimize() if hasattr(domain_trie, 'optimize') else []
        final_domains = []
        
        for domain, mtype in optimized:
            clean = domain.lstrip('*.') if domain.startswith('*.') else domain
            spec_score = domain_trie.get_specificity_score(clean) if hasattr(domain_trie, 'get_specificity_score') else 0
        ...
        if accepted_ips:
            positives = [net for _, net, is_excl, _ in accepted_ips if not is_excl]
            negatives = [net for _, net, is_excl, _ in accepted_ips if is_excl]
    ...
    def _propagate_exclusion(self, exclusions: Dict, accepted: Dict):
        to_remove = []
        
        for (acc_type, acc_val), (acc_w, acc_src, acc_spec, is_excl) in list(accepted.items()):
            if is_excl:
                continue
            
            for (excl_type, excl_val), (excl_w, excl_src) in exclusions.items():
                if self._is_covered_by_exclusion(acc_type, acc_val, excl_type, excl_val):
                    temporal_cmp = self._temporal_compare(acc_src, excl_src)
                    if excl_w >= acc_w * self.config.safety_override_threshold or temporal_cmp > 0:
                        to_remove.append((acc_type, acc_val))
                        break
        
        for key in to_remove:
            del accepted[key]
    
    def _is_covered_by_exclusion(self, acc_type, acc_val, excl_type, excl_val) -> bool:
        if acc_type == 'domain' and excl_type == 'domain':
...
class PersistentLineageAnalyzer:
    __slots__ = ('config', 'state_file', 'wal_backend', '_graph', '_cached_depths', 
                 ...
                 '_global_bdd', '_det_random', '_provenance', '_fuzzer')
    ...
    def compute_incremental(self, parsed_sources: List[ParsedRuleSet]) -> Tuple[Set[int], List[VerifiedSource]]:
        ...
        hash_to_parsed = {s.get_content_hash(): s for s in parsed_sources}
            ...
            for parent in sorted_sources:
                ...
                if is_subset:
                    ...
                    self._provenance.log_activity(
                        'SUBSET_DETECTED',
                        h_child,
                        {'parent': h_parent, 'confidence': confidence, 'reason': reason},
                        'lineage_analyzer',
                        [h_parent]
                    )
        ...
        for i, src in enumerate(indexed_sources):
            ...
            if scc_id is not None:
                ...
                lineage = LineageInfo(
                    ...
                    covering_sources=frozenset(self._graph.get(h, set())) if is_red else None,
    ...
    def _create_scc_unions(self, sources: List[IndexedSource], sccs: List[List[str]], 
                          hash_to_parsed: Dict[str, ParsedRuleSet], max_depths: Dict, min_depths: Dict) -> Dict[int, Optional[VerifiedSource]]:
        union_map = {}
        
        hash_to_indexed = {s.content_hash: s for s in sources}
    ...
    def _compute_depths_with_scc(self, nodes: Set[str], hash_to_parsed: Dict[str, ParsedRuleSet]) -> Tuple[Dict, Dict, Dict, List]:
        if not nodes:
            return {}, {}, {}, []
    ...
    def _apply_cached_results(self, parsed_sources: List[ParsedRuleSet]) -> Tuple[Set[int], List[VerifiedSource]]:
        ...
        for i, parsed in enumerate(parsed_sources):
            ...
            lineage = LineageInfo(
                ...
                covering_sources=frozenset(self._graph.get(h, [])) if is_red else None,
...
def worker(task: Dict, lineage_analyzer: Optional[PersistentLineageAnalyzer] = None, 
    ...
    while retry_count <= max_retries:
        try:
            ...
            task_config_dict = {**asdict(global_config), **task.get('config', {})}
            task_config = MergeConfig(**task_config_dict)
            ...
            try:
                ...
                resolver = StrictConflictResolver(config=task_config)
