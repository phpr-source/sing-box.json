...
| Patch | Effect |
|---|---|
| `urltest-autoban.patch` | **urltest 智能健康淘汰（AutoBan v4.4）**：`auto_ban` 配置块——EWMA 动态健康评分 + 被动单次失败熔断 + 主动指数退避恢复 + 每日固定时段全局大考（`check_times`）+ 群体故障防误杀 + 多目标 HTTPS 204 竞速探针 + TUN Protected Dialer + I/O 防抖 + 多 Group Hash 文件隔离 + `pinned_tags` 手动选择豁免。状态持久化默认 `autoban_<group>_<hash>.json`。本地原创设计（非上游移植） |
